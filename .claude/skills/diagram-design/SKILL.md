...
Installed as skill **`diagram-design`** from upstream
[cathrynlavery/diagram-design](https://github.com/cathrynlavery/diagram-design) (MIT).
Skill root: `.claude/skills/diagram-design/` - run every `python3 scripts/...` from that
directory (or pass absolute paths). Prefer writing finished HTML into the vault under
`attachments/` or `exports/diagrams/`, then embed with a relative markdown link so the
dashboard can open it.

## Javis: khi nào dùng / không dùng

- **Dùng** khi cần sơ đồ editorial đẹp (kiến trúc, flowchart, journey, org, chart…) để
  đọc trên web hoặc xuất HTML/SVG; khi vẽ lại draw.io / Mermaid / Excalidraw; khi cần
  khớp brand tokens.
- **Không dùng** cho sơ đồ tạm trong chat: Mermaid trong markdown là đủ. Không thay
  **`frontend-design`** / **`baseline-ui`** (UI sản phẩm), **`improve-ui`** (audit UI),
  **`create-design-md`** (ghi DESIGN.md), **`brainstorming`** (spec trước khi dựng).
- Bộ UI/UX (`ui-skills-root`, `frontend-design`, …) **ủy quyền** sang skill này khi brief
  yêu cầu diagram thị giác, không tự viết SVG ad-hoc.
