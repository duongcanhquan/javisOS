...
(function () {
  ...
  function renderDomainStatus(j) {
    ...
    var steps = '<div class="dom-step done"><span class="dom-step-num">' + ic("check") + '</span><div><b>1. Lưu tên miền</b><p>VMOS đã ghi nhận <code>' + esc(j.domain) + '</code>.</p></div></div>';
    ...
    if (hostinger) {
      ...
      steps += '<div class="dom-step ' + (j.ssl_active ? "done" : (j.dns_ok ? "warn" : "")) + '"><span class="dom-step-num">' + (j.ssl_active ? ic("check") : "3") + '</span><div><b>3. Bật HTTPS</b>' +
        '<p>' + (j.ssl_active ? "Chứng chỉ HTTPS đang hoạt động." : "Khi DNS đã đúng, bấm Bật SSL để VMOS xin chứng chỉ.") + '</p>' +
    ...
    else if (hostinger && j.requires_redeploy) setStatus("domainStatus", "Đã lưu trong VMOS; còn bước đặt DOMAIN_NAME và Redeploy trên Hostinger.", false);
