// ============================================================
// VMOS - Trang Chatbot: quản lý các Bot chuyên trách (mỗi bot một Agent, một brain,
...
(function () {
  ...
  // Multi-user: nhịp yên 12s; khi đang khởi động thì 5s để thẻ kịp đổi trạng thái.
  var NHIP_CO_TIN = 5000;
  var NHIP_YEN = 12000;
  var _nhipMs = NHIP_YEN;
  ...
  function dieuChinhNhipBot(ds) {
    var dangKhoiDong = (ds || []).some(function (b) {
      var st = (b && b.status) || {};
      return st.status === "starting" || st.status === "polling";
    });
    var ms = dangKhoiDong ? NHIP_CO_TIN : NHIP_YEN;
  ...
  function the(b) {
    ...
    // Chế độ riêng tư của Telegram chặn Ở PHÍA TELEGRAM, trước khi VMOS nhìn thấy tin nào.
      ...
      '<div class="cb-modal"><div class="cb-form cb-wizard">' +
        ...
        '<div class="cb-b" id="cbB2">' +
          ...
          '<details class="cb-nangcao">' +
            ...
            // bot nói chuyện với NGƯỜI NGOÀI, không phải với chủ. Chủ dùng VMOS bằng tiếng
