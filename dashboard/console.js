// ============================================
// LYON - Console layer (sidebar + router)
...
(function () {
  ...
  function navigateTo(id) {
    ...
    const swap = () => {
      ...
      // Nút điều khiển cockpit (cài đặt, giọng nói, làm mới) chỉ hiện ở trang LYON, không hiện navbar trang quản lý
  ...
  // ============================================
  // Trang Mức dùng (token & chi phí LYON tự đo, có đồ thị 14 ngày)
  // ============================================
  let _uzCss = false;
  ...
  async function renderUsage(el) {
    ...
    el.innerHTML = `<div class="uz-wrap">${cards}${chart}${table}
      <div class="uz-note">Số liệu do LYON tự đo từ token vào/ra của mọi engine (Claude Code, ChatGPT/Codex, OpenRouter...), không phụ thuộc nhà cung cấp có lộ hạn mức hay không. Chi phí chỉ hiện khi nhà cung cấp trả về giá thật (vd Claude Code CLI); còn lại chỉ đếm token. Lưu 30 ngày gần nhất.</div>
    </div>`;
  }
  ...
  async function renderLogs(el) {
    ...
    el.innerHTML = `<div class="cl-wrap">
      <section class="upd-card" aria-label="Cập nhật LYON">
        <div class="upd-title"><span class="upd-name">LYON</span><span class="gcard-tag" id="updVerTag">…</span></div>
  ...
  async function renderFiles(el) {
    ...
    async function load(path) {
      ...
      if (!resp.ok || d.error) {
        const msg = d.error || (resp.status === 404
          ? "Máy chủ LYON chưa có chức năng Tệp tin - hãy KHỞI ĐỘNG LẠI server (stop-javis.bat → start-javis.vbs) rồi tải lại trang."
    ...
    function veBangSua(items) {
      ...
      fixEl.innerHTML = `<div class="fm-fix">${WARN_ICON}
        ...
        Bản LYON trước 0.33.4 lưu note qua trình sửa trực quan là làm hỏng khối thuộc tính
  ...
  async function renderPlugins(el) {
    ...
    async function load() {
      ...
      if (myGen !== _renderGen) return;
      const intro = `<p style="color:var(--text3);font-size:15px;max-width:720px;margin:0 0 12px">Plugin thêm <b>tool</b> (công cụ engine gọi được) và <b>hook</b> native cho LYON mà không sửa lõi - dùng được ở MỌI engine (Claude Code, Codex, API) qua hub, tôn trọng 3 mức quyền như tool khác.</p>`;
      const gateBanner = (!d.user_gate) ? `<div style="margin-bottom:14px;padding:11px 13px;border:1px solid rgba(224,160,74,.5);border-radius:10px;background:rgba(224,160,74,.08);color:var(--warn-ink);font-size:13px;line-height:1.55"><b>${WARN_ICON} Plugin do bạn cài đang bị chặn.</b> Plugin toàn cục/brain chạy code Python thật trong server nên mặc định TẮT. Để bật: đặt biến môi trường <code>JAVIS_ENABLE_USER_PLUGINS=true</code> rồi khởi động lại LYON. Plugin có sẵn (bundled) vẫn chạy bình thường.</div>` : "";
      const dirHint = `<p style="color:var(--text3);font-size:12.5px;margin:0 0 14px">Thả plugin TOÀN CỤC (dùng cho MỌI brain) vào <code>${esc(d.global_dir || "")}</code> · mỗi plugin gồm <code>plugin.yaml</code> + <code>plugin.py</code>. Hoặc bảo LYON trong khung chat: "tạo plugin ...".</p>`;
  ...
  async function renderSelfImprove(el) {
    ...
    el.innerHTML = `<div class="cview-section"><div class="empty">Đang tải...</div></div>`;
    const GNAME = { business: "Kinh doanh", brain: "Bộ não", product: "Cải thiện LYON", custom: "Tự định nghĩa" };
    ...
    el.innerHTML = `<div class="cview-section">
      ...
      <div id="lpForm" style="display:none;margin-bottom:14px;padding:14px;border:1px solid var(--hairline);border-radius:10px;background:var(--surface-1)">
        ...
        <div class="si-grid">
          ...
          <div class="si-field"><label id="lpBodyLabel">Mô tả nhiệm vụ (mỗi vòng LYON làm đúng việc này)</label>
          ...
          <div id="lpRemFields" style="display:none">
            ...
            <div class="dim" style="font-size:12px;color:var(--text3);margin-top:4px">Nhắc một lần: "30 phút nữa", "8h30", "2026-07-20 09:00". Lặp theo giờ cố định: cron 5 trường (vd "0 7 * * *" = 7h sáng mỗi ngày). "Chỉ nhắc" = bắn tin nhắc bạn; "Tự làm rồi báo" = LYON chạy đúng việc này rồi báo kết quả.</div>
    ...
    function syncKindUI() {
      ...
      q("#lpBodyLabel").textContent = isRem
        ? "Nội dung nhắc (LYON sẽ nhắc hoặc làm đúng việc này)"
        : "Mô tả nhiệm vụ (mỗi vòng LYON làm đúng việc này)";
    }
    ...
    el.querySelector("#lpSave").onclick = async () => {
      ...
      if (!body) { msg.textContent = fkind === "reminder" ? "Nhập nội dung nhắc" : "Nhập mô tả nhiệm vụ (LYON cần biết mỗi vòng làm gì)"; return; }
    ...
    async function loadAll(retried) {
      ...
      groups.forEach(g => {
        ...
        if (!loops.length && !rems.length) {
          ...
          e2.dataset.lp = "empty";
          e2.innerHTML = `Chưa có việc nào ở brain này. Bấm <b>+ Thêm việc</b>, hoặc nói với LYON trong chat.`;
      ...
      if (!anyItem) {
        box.innerHTML = `<div class="empty">Chưa có việc định kỳ hay nhắc hẹn nào. Bấm <b>+ Thêm việc</b>, hoặc nói với LYON trong chat (vd "tạo loop mỗi 2 tiếng đọc 1 source rồi đề xuất").</div>`;
      }
  ...
  async function renderLearn(el) {
    ...
    el.innerHTML = `<div class="cview-section">
      ...
      <div class="si-log"><h3 style="font-size:15px;color:var(--text)">LYON đã tự học gì (commit gần nhất)</h3><div id="lnReview">Đang tải...</div></div>
  ...
  async function renderOverview(el) {
    ...
    // ghi là "Claude CLI". Mọi provider đều có MCP LYON, khác nhau ở chỗ chạy được lệnh máy.
    const _mainP = (m.providers || []).find(p => p.id === (m.main || {}).provider) || {};
    const eng = (_mainP.label || (m.main || {}).provider || "-")
      + (_mainP.kind === "api" ? " (MCP LYON)" : _mainP.kind ? " (MCP LYON + lệnh máy)" : "");
    ...
    el.innerHTML = `
      <div class="cview-section">
        ...
        <div class="gcard" style="max-width:640px">
          <div class="gcard-top"><span class="gcard-name">LYON</span><span class="gcard-tag" id="ovVerTag">…</span></div>
      ...
      <div class="cview-section">
        ...
        <div class="cgrid">
          ...
          <div class="gcard"><div class="gcard-top"><span class="gcard-name">Workspace</span></div><div class="gcard-meta">${esc(s.workspace_name || "LYON")}</div></div>
      ...
      <div class="cview-section" id="ovAutostartSec" style="display:none">
        ...
        <div class="cgrid">
          <div class="gcard">
            <div class="gcard-top"><span class="gcard-name">Tự bật LYON khi mở máy</span><span class="gcard-tag" id="ovAutoTag">…</span></div>
    ...
    async function ovLoadAutostart() {
      ...
      meta.innerHTML = on
        ? "LYON tự chạy nền mỗi khi bạn đăng nhập Windows - không cần bật tay. Chạy ẩn, mở <code>localhost:7777</code> để dùng."
        : "Bật để LYON tự khởi động mỗi khi mở máy. Chạy ẩn ở nền, không hiện cửa sổ.";
  ...
  async function renderModels(el) {
    ...
    const provCard = (p) => {
      ...
      const cliWarn = (ten) => p.cli_found === false
        ? `<div class="prov-note warn">${WARN_ICON} <b>Máy chạy LYON chưa có lệnh <code>${ten}</code></b>
             - ${on ? "đăng nhập đã xong nhưng" : ""} chat bằng thẻ này sẽ lỗi cho tới khi cài.
             Bản Docker: <b>cập nhật LYON lên bản mới nhất</b> là có sẵn. Bản cài tay: mở trang
             Code &gt; Terminal chạy <code>${esc(p.cai_lenh || "")}</code> rồi khởi động lại LYON.</div>`
        : "";
      if (p.kind === "oauth") {
        const st = on
      ...
      if (p.id === "antigravity-cli") {
        ...
        return `<div class="prov-card ${p.is_main ? "main" : ""}">
          ...
          ${p.cli_found ? "" : `<div class="prov-steps">
            <div>Chưa thấy CLI trên máy. Cài một lần trên máy chạy LYON:<br><code>${esc(p.cai_lenh || "")}</code></div>
          </div>`}
          ${on ? "" : `<div class="prov-steps">
            <div><b>Đăng nhập trong terminal của máy chạy LYON:</b> <code>${esc(dn.dang_nhap || "agy")}</code></div>
      ...
      if (p.id === "gemini-cli") {
        ...
        return `<div class="prov-card ${p.is_main ? "main" : ""}">
          ...
          <div class="prov-note warn"><b>Google đã ngắt đường này với tài khoản cá nhân từ 18/06/2026</b>
            ...
            <code>IneligibleTierError</code>. Chặn từ phía Google, không sửa được bên LYON.
      ...
      if (p.kind === "cli") {   // Claude Code - trạng thái + login/logout nạp động qua /claude/status
        ...
        // phiên cũ); khác nhau ở chỗ ai trả tiền và ai chịu rủi ro. LYON cố ý không tắt cứng
      ...
      return `<div class="prov-card ${p.is_main ? "main" : ""}">
        ${provHead(p, on, p.kind === "cli" ? "MCP/skill" : "MCP LYON", (on ? "● Đã kết nối" : "○ Chưa kết nối") + " · " + p.models.length + " model")}
        ${p.needs_key
    ...
    el.innerHTML = `
      <div class="cview-section">
        ...
        <div class="gcard current" style="max-width:540px">
          ...
          <div class="gcard-meta">${mainP.id === "gemini-cli" ? "Qua Gemini CLI - MCP LYON + skill + loop + chạy lệnh máy (đăng nhập Google)"
            : mainP.kind === "cli" ? "Qua Claude Code - MCP LYON + skill + loop + chạy lệnh máy"
            : mainP.kind === "oauth" ? "Qua Codex - MCP LYON + skill + loop + chạy lệnh máy"
            : mainP.kind === "api" ? "Gọi API thẳng - MCP LYON + skill + loop (không chạy lệnh máy)" : ""}</div>
    ...
    if (glo) glo.onclick = async () => {
      if (!confirm("Ngắt tài khoản Google khỏi LYON?")) return;
  ...
  // Codex quyết, LYON cố ý không ghim version), nên trên máy mới đăng nhập xong là thẻ hiện
  ...
  // cả khi LYON nằm trên VPS còn trình duyệt ở máy người dùng - y như đăng nhập Claude Code.
  ...
  async function startOauthBrowser(el) {
    ...
    if (d.error) { if (msg) msg.textContent = "Lỗi: " + d.error; return; }
    if (!d.authorize_url) { if (msg) msg.textContent = "Máy chủ chưa có chức năng này - khởi động lại LYON rồi tải lại trang (Ctrl+Shift+R)."; return; }
  ...
  function openApikeyFlow(el, con, isFirst, ctx) {
    ...
    m.querySelector("#cGo").onclick = async () => {
      ...
      m.querySelector(".conn-form").innerHTML = '<div class="conn-ok">' + CHECK_ICON + ' Đã kết nối: <b>' + esc(r.label || con.name) + '</b> (' + (r.tools || 0) + ' công cụ)'
        + (isFirst ? '<div class="conn-hint">Sang trang LYON hỏi thử: "Hôm nay bán được bao nhiêu?"</div>' : "") + '</div>';
      ...
      _connPoll = setInterval(async () => {
        ...
        if (st.state === "qr" && st.qr) {
          ...
          zone.innerHTML = '<div class="conn-ok">' + CHECK_ICON + ' Đã đăng nhập: <b>' + esc(st.label || "Zalo") + '</b>'
            + (isFirst ? '<div class="conn-hint">Sang trang LYON nhắn thử: "Đọc tin nhắn Zalo mới nhất"</div>' : "") + '</div>';
  ...
  function openFullAck(el, c, con) {
    const text = (con && con.risk) ? con.risk
      : "Mức này cho phép LYON thao tác THẬT ra ngoài qua kết nối này: tạo đơn, gửi tin, chạy quảng cáo, publish… Hành động có thể KHÔNG hoàn tác được.";
  ...
  function openAccountMenu(el, c, con) {
    ...
    m.querySelectorAll("[data-m]").forEach(b => b.onclick = async () => {
      ...
      if (act === "test") {
        ...
        // Nguồn tự giữ token ngoài LYON (workspace-mcp): nút Kết nối lại chỉ lưu key chứ không
        ...
        if (!confirm('Xoá đăng nhập Google của "' + (c.label || "") + '"?\n\n'
          + 'Kết nối giữ nguyên. Lần sau nhờ LYON làm việc với nguồn này, trình duyệt trên MÁY '
          + 'CHẠY LYON sẽ mở để bạn cấp lại quyền - nhớ tick hết các ô.')) return;
  ...
  async function renderConnect(el) {
    ...
    // MỌI provider LYON hỗ trợ đều gọi được kho Kết nối: CLI (Claude, Codex, Antigravity,
    ...
    if (main.provider === "openai-oauth") {
      warn = `<div class="gx-alert gx-alert-ok"><div class="gcard-meta" style="opacity:1">${CHECK_ICON} <b>ChatGPT (gói subscription)</b> chạy qua <b>Codex CLI</b> - LYON tự đẩy kho Kết nối sang Codex qua hub, nên vẫn dùng được đầy đủ.</div></div>`;
    } else if (main.provider === "antigravity-cli") {
      warn = `<div class="gx-alert gx-alert-ok"><div class="gcard-meta" style="opacity:1">${CHECK_ICON} <b>Antigravity CLI</b> (<code>agy</code>) dùng được kho Kết nối qua <b>MCP LYON</b> + skill + lệnh máy. Không có WebSearch sẵn như Claude Code - tra web phải qua MCP đã đấu (vd Search Console cho SEO site của bạn).</div></div>`;
    } else if (main.provider === "gemini-cli") {
      warn = `<div class="gx-alert gx-alert-ok"><div class="gcard-meta" style="opacity:1">${CHECK_ICON} <b>Gemini CLI</b> dùng được kho Kết nối qua <b>MCP LYON</b> + skill + lệnh máy. Google đã ngắt hạng cá nhân 18/06/2026 - nên dùng <b>Antigravity CLI</b> nếu còn lỗi đăng nhập.</div></div>`;
      ...
      warn = `<div class="gx-alert gx-alert-ok"><div class="gcard-meta" style="opacity:1">${CHECK_ICON} <b>${esc(mainLabel)}</b> dùng được kho Kết nối qua <b>MCP LYON</b> (vòng gọi tool + hub), kèm tool file trong brain và skill - không phải chat suông.</div></div>`;
    }
    const groups = {};
    ...
    el.innerHTML = '<div class="galaxy-nexus" id="galaxyNexus">'
      ...
      + '<div class="gcard-meta gx-zone-meta">Một dịch vụ nối được NHIỀU tài khoản (nhiều shop, nhiều số Zalo…). Mọi bộ não dùng chung kho này qua trung tâm kết nối của LYON, kèm phân quyền và nhật ký.'
      + '<label class="gx-strict"><input type="checkbox" id="mcpStrict" ' + (d.strict ? "checked" : "") + '> Chỉ dùng kết nối của LYON (bỏ kết nối sẵn của máy)</label></div>'
      ...
      + '<div class="gcard-meta gx-zone-meta" style="margin-top:10px">Nguồn đã đăng nhập trong Claude / Codex CLI. Bộ não tương ứng tự dùng được khi trạng thái Connected. Quản lý ngoài LYON.</div>'
  ...
  async function renderChannels(el) {
    ...
    el.innerHTML = `
      ...
      <div class="cview-section">
        ...
        <div class="gcard" style="max-width:560px">
          <div class="gcard-meta" style="margin-bottom:8px">Bot Zalo <b>chính thức</b> để hỏi LYON
            từ điện thoại. Khác <b>Zalo Agent MCP</b> ở trang Kết nối: cái kia đăng nhập chính tài
            khoản của bạn để LYON thao tác thay bạn, cái này là một danh tính riêng, an toàn, để
            bạn nhắn cho LYON.</div>
    ...
    async function refreshTgStatus() {
      ...
      if (!d.enabled) line = ic("circle", { cls: "ic-dim" }) + " Bot CHƯA bật - tích 'Bật bot Telegram' rồi Lưu (test gửi được KHÔNG có nghĩa bot đang nhận tin).";
        ...
        line = `${ic("circle", { cls: "ic-fill ic-ok" })} Bot đang nhận tin - ${n ? n + " chat ID được phép" : "MỌI NGƯỜI nhắn được (chưa giới hạn ID)"} - nhắn cho bot là LYON trả lời.`;
      }
  ...
  async function renderAccount(el) {
    ...
    el.innerHTML = `
      <div class="cview-section">
        ...
        <div class="gcard" style="max-width:560px">
          ...
          <input class="js-input" id="acWs" value="${esc(s.workspace_name || "LYON")}">
      ...
      <div class="cview-section">
        ...
        <div class="gcard" style="max-width:560px">
          <div class="gcard-meta">Token để <b>LYON CLI</b> (hoặc script) gọi được LYON từ máy khác. Không có token nào sẵn - chưa tạo thì không đường nào vào ngoài trình duyệt.</div>
          ...
          <div class="tk-docs">
            <a href="https://github.com/blogminhquy/javis-os/blob/main/docs/24-cli-terminal.md" target="_blank" rel="noopener">Hướng dẫn LYON CLI ↗</a>
    ...
    document.getElementById("acWsSave").onclick = async () => {
      ...
      const _ws = document.getElementById("acWs").value.trim() || "LYON";
  ...
  async function renderTfa(rootEl) {
    ...
    if (a.totp_enabled) {
      ...
      document.getElementById("tfaOff").onclick = async () => {
        if (!confirm("Tắt xác thực 2 lớp? Từ đó chỉ còn mật khẩu bảo vệ LYON.")) return;
        const fd = new FormData();
    ...
    head.innerHTML = a.totp_suggested
      ...
      : ic("shield") + " Chưa bật. Bật thì mật khẩu lộ ra ngoài cũng chưa đủ để vào được LYON.";
  ...
  //
  // Vì sao cần: LYON có HAI bề mặt cài đặt tài khoản - trang Tài khoản (đủ thứ, gồm cả 2FA)
  // và khối cũ này nhúng trong trang Cài đặt (chỉ đổi mật khẩu). Ai mở Cài đặt trước sẽ thấy
  // một khối tài khoản không nhắc gì tới 2FA và kết luận LYON không có, rồi thôi.
  //
  ...
  async function renderTfaRow() {
    ...
    row.innerHTML = a.totp_broken
      ...
      : `${ic("shield")} Xác thực 2 lớp: <b class="tfa-off">chưa bật</b>`
        + ` - bật thì mật khẩu lộ ra ngoài cũng chưa đủ để vào được LYON.`
  ...
  async function renderSettings(el) {
    ...
    el.innerHTML = `<div class="settings-page">
      <details class="settings-group" open>
        ...
        <div class="settings-group-body">
          <div class="settings-status-grid">
            ...
            <div><span>Workspace</span><b>${esc(s.workspace_name || "LYON")}</b></div>
      ...
      <details class="settings-group" id="setAutostartSec" style="display:none">
        <summary><span><b>Khởi động cùng Windows</b><small>Tự chạy LYON ở nền khi đăng nhập</small></span><span class="settings-caret">${ic("chevron-down")}</span></summary>
        <div class="settings-group-body">
          <div class="settings-card compact">
            <div class="settings-card-head"><b>Tự bật LYON</b><span class="gcard-tag" id="setAutoTag">…</span></div>
    ...
    if (langHost) {
      ...
      if (selUi) selUi.onchange = async () => {
        ...
        // THEO THIẾT BỊ (người dùng mở LYON từ nhiều máy), nên trải nghiệm phải tức thì và
    ...
    const setC2pa = async (strip) => {
      if (strip && !confirm("Gỡ dấu nguồn gốc AI khỏi ảnh LYON tạo?\n\n"
    ...
    const loadAutostart = async () => {
      ...
      document.getElementById("setAutoMeta").innerHTML = (on
        ? "LYON tự chạy nền khi bạn đăng nhập Windows; mở <code>localhost:7777</code> để dùng."
        : "Bật để LYON tự khởi động ở nền mỗi khi mở máy.")
  ...
  // rời trang. Cùng một cuộc trò chuyện hiển thị ở cả màn LYON lẫn tab này.
  // ============================================
  ...
  function _injectChatCss() {
    ...
    /* Khung nhập giữ NGUYÊN bộ mặt của thanh nhập ở màn LYON (--bg2 + bo 18px). Trước đây
    ...
    /* Màn hẹp: cả hàng tiêu đề phải nằm gọn MỘT dòng. Trước đây tiêu đề "Trò chuyện với LYON"
  ...
  // chủ repo chỉ ra ngay: "sao không bê nguyên cái cây y hệt bên LYON sang mà phải dựng lại".
  ...
  function renderChat(el) {
        ...
        '<div class="chatpage-main">' +
          '<div class="chatpage-bar">' +
            ...
            '<button class="cp-ico-btn cp-min" type="button" id="cpMinBtn" ' +
              'title="Thu nhỏ về màn LYON" aria-label="Thu nhỏ về màn LYON">' +
              ic("chevron-left") + '<span>Thu nhỏ</span></button>' +
            '<span class="cp-title">Trò chuyện với LYON</span>' +
    ...
    // Đường VỀ. Nút phóng to ở màn LYON nay dẫn thẳng sang trang này (lớp nổi .chat-stage đã
  ...
  async function _vtSearchContent(q) {
    ...
    if (!resp || resp.status === 404) {
      box.innerHTML = `<div class="vr-empty">Tìm theo <b>nội dung</b> cần khởi động lại LYON một lần (chạy start-javis.bat) để bật. Tạm thời hãy tìm theo <b>Tên</b>.</div>`;
      return;
    }
  ...
  async function _neDeleteCur(rel, it) {
    ...
    const fd = new FormData(); fd.append("brain", fbrain()); fd.append("path", rel);
    try { await fetch("/files/delete", { method: "POST", body: fd }); } catch (e) {}
    // Ghim trỏ tới file vừa xoá thì bỏ, đừng để LYON đi mở một đường dẫn không còn tồn tại.
    try {
  ...
  // rời đi cũng bị tính là có sửa, và LYON sẽ âm thầm ghi đè định dạng của file đó.
  ...
  // dùng được LYON, và một việc phải làm là vào Models.
  async function refreshEngineBanner() {
    ...
    b.title = `Chưa dùng được: ${name} - ${rec.message || "không phản hồi"}. `
      + "Vào trang Models để kết nối và sử dụng LYON.";
    b.hidden = false;
  }
  ...
  function boot() {
    ...
    // đang đứng ở màn LYON mà tự bị đẩy sang Trò chuyện là mất chỗ đang xem)
    ...
    if (gs) gs.addEventListener("change", () => {
      ...
      // Ghim của brain cũ trỏ ra ngoài brain mới → bỏ, kẻo LYON sửa nhầm file brain khác.
    ...
    freshSettings().then(s => {
      ...
      // MỞ APP LÀ VÀO MÀN LYON, kể cả lite-mode (cờ graph tắt hoặc màn hẹp): màn LYON đã có
