...
(function () {
  ...
  function applyMicAec(on) {
    ...
    for (s = 0; s < streams.length; s++) {
      ...
      if (mic && mic.applyConstraints) {
        try {
          mic.applyConstraints(moonshineMicConstraints());
        } catch (e) {}
      }
    }
  }
   ...
   * Chuỗi phổ thông: ghi NGAY (Cloud STT / Web Speech). Moonshine chỉ khi engine đã có trong RAM.
   */
  ...
  /**
   * Desktop + COOP/COEP OK + model local: Moonshine khi ĐÃ nạp xong (không chặn Bắt đầu).
  ...
  function moonshineEngineReady(lang) {
  ...
  var MOONSHINE_CAPTURE_WORKLET =
    ...
    "var mono=downmixToMono(input),i=0;" +
    "while(i<mono.length){var space=this._buf.length-this._n,take=Math.min(space,mono.length-i);" +
    "this._buf.set(mono.subarray(i,i+take),this._n);this._n+=take;i+=take;" +
    ...
    if (preferMoonshineFirst(lang)) {
      if (state.moonshineReady && state.moonshineLang === lang) {
        el.textContent =
          "Moonshine sẵn sàng (" +
          label +
          ") — Bắt đầu dùng engine local, không tải lại model.";
