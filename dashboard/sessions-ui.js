// ============================================================
// VMOS - Sidebar "Lịch sử hội thoại" TRONG chat workspace (cột trái khi phóng to chat).
...
(function () {
  ...
  // project A trong khi cột trái đang lọc project B thì VMOS vẫn nhận hướng dẫn của A. Chip
  ...
  //
  // Danh sách của cuộc trộn HAI nguồn: phần VMOS tự dò ra từ tin nhắn (`manual: false`,
  ...
  function hangMuc(o) {
    // Hàng TỰ DÒ (chỉ có ở chế độ cuộc): VMOS suy ra từ tin nhắn chứ không phải một bản ghi,
  ...
  function dsFileHtml(p) {
    var fs = p.files || [];
    var laCuoc = pdLaCuoc();
    // Chế độ cuộc: file người dùng GẮN TAY (có id) đứng trên, file VMOS tự dò ra đứng dưới.
    ...
    var ve = function (f) {
      ...
      return hangMuc({ id: f.id, ten: f.label || f.name || f.path,
                       ...
                       // đối trên máy chủ: đó mới là chuỗi mọi nơi khác của VMOS nhận vào
  ...
  function veNutKetQua() {
    ...
    if (!box || !p) return;
    var theoDuong = {};
    // Chỉ file GẮN TAY mới đối chiếu được: file VMOS tự dò ra không có id nên không gỡ được,
  ...
  // mà chip còn đứng ở màn VMOS nơi cột đó chưa bao giờ mount. Đổi phiên là đổi project có
