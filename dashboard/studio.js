...
(function () {
  ...
  function renderWorkflowUI() {
    ...
    panel.innerHTML = `<div class="panel-bar"><h3>Workflows</h3><div class="pb-actions"><button class="s-btn-ghost" id="wfSelAll" title="${esc(t("studio.selall_title"))}">${esc(t("studio.selall"))}</button><button class="s-btn-ghost" id="wfDl" disabled title="${esc(t("studio.dl_title"))}">${esc(t("studio.dl_sel"))}</button><button class="s-btn-ghost" id="wfImport">${esc(t("studio.import"))}</button><button class="s-btn-ghost" id="seedBtn">${esc(t("studio.seed"))}</button><button class="s-btn-ghost" id="seedStrategyBtn" title="NCTT → chiến lược KD/MKT → proposal">Bộ Proposal</button><button class="s-btn-ghost" id="seedVideoBtn" title="Nghiên cứu → kịch bản → đạo diễn đa pipeline">Bộ Video</button><button class="s-btn-ghost" id="seedBaiGiangBtn" title="Bài giảng Gemini: lớp học / video / slide / văn bản">Bộ Bài giảng</button><button class="s-btn-ghost" id="seedTruongBtn" title="Gói trường: Bộ Bài giảng + hướng dẫn giáo viên">Bộ Trường</button><button class="s-btn-ghost" id="seedMarketingBtn" title="Marketing Gemini: SEO / nghiên cứu / Facebook">Bộ Marketing</button><button class="s-btn-ghost" id="seedPhapCheBtn" title="Agent Pháp chế + sources/phap-che">Bộ Pháp chế</button><button class="s-btn" id="newWf">+ Workflow</button></div></div>
    ...
    document.getElementById("seedTruongBtn") && (document.getElementById("seedTruongBtn").onclick = async () => {
      const btn = document.getElementById("seedTruongBtn");
      if (btn) { btn.disabled = true; btn.textContent = "Đang tạo…"; }
      try {
        await api("/studio/seed-truong", { method: "POST", body: fd({ brain: brain() }) });
        loadWorkflows(); loadAgents?.();
      } finally {
        if (btn) { btn.disabled = false; btn.textContent = "Bộ Trường"; }
      }
    });
