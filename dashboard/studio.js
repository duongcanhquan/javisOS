...
(function () {
  ...
  function renderWorkflowUI() {
    ...
    document.getElementById("seedPosterBtn").onclick = async () => {
      const btn = document.getElementById("seedPosterBtn");
      if (btn) { btn.disabled = true; btn.textContent = "Đang tạo…"; }
      try {
        await api("/studio/seed-poster", { method: "POST", body: fd({ brain: brain() }) });
        loadWorkflows(); loadAgents?.();
      } finally {
        if (btn) { btn.disabled = false; btn.textContent = "Bộ Poster"; }
      }
    };
