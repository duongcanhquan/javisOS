...
(function () {
  ...
  var SRC_LABEL = { manual: "Bạn gõ tay", javis: "VMOS (tự chạy)" };
  ...
  // fetch KHÔNG reject với status 4xx/5xx, mà middleware đăng nhập của VMOS lại trả 401 kèm
  ...
  function mucHtml(d) {
    ...
    var chuaChon = d.tu_chon ? "" : " Mức đang chạy là mặc định của bản này, bạn chưa tự chọn bao giờ."
      + " Bấm một mức bất kỳ là VMOS ghim lại, từ đó không bản cập nhật nào đổi nữa.";
    ...
    var body = state.moMuc
      ...
      + doHtml
      + '<div class="tk-muc-note">Đổi xong có hiệu lực ngay, không cần khởi động lại. Thấy VMOS'
  ...
  function heroHtml() {
    ...
    if (!((t.tien || {}).goi || {}).gia_thang_usd) {
      phu.push("Khai giá gói mỗi tháng ở ô Tiền mặt bên dưới thì VMOS tính được gói đang lời hay lỗ.");
    }
  ...
  function oTran() {
    ...
    if (c.tran_khai) {
      ...
      body += '<div class="s">Khai trần gói ở ô Tiền mặt thì VMOS báo được lúc sắp bị chặn.</div>';
    }
    if (!thueBao) {
  ...
  function oTietKiem() {
    ...
    if (!k.du_du_lieu) {
      ...
      // tiết kiệm đi: "chưa chạy lượt nào" với "bạn đang chỉnh tay nên VMOS không biết cấu
      ...
      return '<div class="tk-a"><div class="h">' + ic("sparkles", { cls: "ic-sm" }) + "Đã tiết kiệm</div>"
        ...
        + (k.khong_do_duoc
          ? "Bạn đang tự chỉnh tay từng đường nên VMOS không biết cấu hình này tốn bao nhiêu mỗi lượt. Chọn một mức có sẵn là đo được."
          : "Chưa có lượt nào của VMOS trong kỳ này để tính.")
        + "</div></div>";
    }
    var ti = k.tien || {};
    return '<div class="tk-a"><div class="h">' + ic("sparkles", { cls: "ic-sm" }) + "Đã tiết kiệm</div>"
      + '<div class="big warn">' + fTok(k.token) + "</div>"
      + '<div class="s">token không phải gửi, qua ' + (k.so_luot || 0) + " lượt của VMOS. "
  ...
  function nganSachHtml() {
    ...
    return '<div class="tk-ns"><div class="row">'
      ...
      + esc(val(f.h5, c.tran_khai)) + '" placeholder="Để trống thì VMOS tự đo"></div>'
      + "</div>"
      ...
      + '<div class="hint">Bốn con số này VMOS không tự biết được: nhà cung cấp không cho lấy giá gói'
  ...
  function paint(s, insights) {
    ...
    var grid = '<div class="tk-grid">'
      + "<div><div class=\"tk-sec\">Nguồn tiêu (bạn vs VMOS)</div>" + barList(s.by_source, SRC_LABEL, function () { return "var(--accent)"; })
