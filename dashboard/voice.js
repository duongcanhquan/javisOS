// ============================================
// VMOS - Voice Layer (Web Speech API)
// ============================================
...
class JavisVoice {
  ...
  // kẹt giữa hai cái sai: ngưỡng thấp thì VMOS nghe chính mình rồi tự câm, ngưỡng cao thì
  ...
  constructor(opts = {}) {
    ...
    this.audioCtx = null;
    this.outAnalyser = null;   // âm VMOS đọc (TTS)
  ...
  _initRecognition() {
    ...
    // phiên: nói xong -> gửi -> vòng rảnh tay bên app.js mở lại khi VMOS đọc xong.
    ...
    this.recognition.onstart = () => {
      ...
      // lại MỞ vĩnh viễn và onend còn tự khởi động lại. Người dùng tưởng đã tắt, thực ra VMOS
    ...
    this.recognition.onresult = (event) => {
      // Đang phát TTS thì BỎ mọi kết quả nhận dạng: đó là mic nghe lại chính giọng VMOS
      // (SpeechRecognition thu riêng, KHÔNG được khử vọng như luồng đo mức âm), không phải
      // user nói. Không chặn thì giọng VMOS bị chép vào khung chat rồi tự gửi đi.
      ...
      if (display) {
        ...
        // lồ và VMOS trông như điếc suốt cả đoạn đó (chủ dự án 17/09 gửi ảnh một lượt dài cả
        ...
        // VMOS, còn câu thật thì được trả lời trong vòng nửa phút chứ không phải hai phút.
  ...
  // chỉ sống trong lúc VMOS ĐỌC, là lúc nhận dạng đã bị abort (xem _muteRecognition), nên
  ...
  // VMOS bắt đầu đọc mà mic đang nghe → tạm NGỪNG nhận dạng (abort, bỏ kết quả dở),
  ...
  // Đang nói chuyện bằng giọng thì phải GIỮ luồng mic sống trong lúc VMOS đọc, kể cả khi
  ...
  _startBargeMonitor() {
    ...
    // 15/09: nói chen vào, VMOS đọc tiếp như không nghe thấy gì).
    //
    // Nỗi lo cũ vẫn được giữ nguyên, chỉ đổi chốt cho đúng chỗ: không được để luồng mic sống
    // suốt đời trang rồi lần nào VMOS đọc cũng rình, vì một tiếng động to trong phòng (nhạc,
    ...
    // không". Rảnh tay tắt thì VMOS đọc trong im lặng, không nghe gì hết.
    ...
    // là phòng im và ngưỡng rơi về sàn 0.045: loa ngoài mở to là vượt ngay, VMOS tự ngắt
  ...
  _ketThucNha(laNguoi, preLevel, dungSynth) {
    ...
    // thẳng là nền treo ở đó, ngưỡng vọt lên gần 0.4 và mấy lượt sau nói gì VMOS cũng không
  ...
  // Phần VMOS ĐÃ ĐỌC RA TIẾNG trong lượt này: các khúc đã phát xong cộng phần khúc dở theo
  ...
  _chunkFailed(i, retry) {
    ...
    if (okBrowserVoice) this._speakBrowser(this.ttsChunks[i], () => this._playChunk(i + 1));
      ...
      try { console.warn("[VMOS TTS] bỏ khúc (backend hỏng, máy không có giọng Việt):", this.ttsChunks[i]); } catch (e) {}
