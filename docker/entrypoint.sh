...
# người dùng tự thêm, dù đăng nhập Google vẫn còn. Với GEMINI_FORCE_FILE_STORAGE=true trên
# Docker/headless, agy lưu token ra file thay vì keyring.
#
...
for d in .local .antigravity .config .gemini .grok .copilot; do
    src="$HOME/$d"
    dst="$PERSIST_ROOT/$d"
