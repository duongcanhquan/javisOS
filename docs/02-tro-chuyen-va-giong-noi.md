...
Trong lúc Javis suy nghĩ, một chip hoạt động hiện ngay cuối khung chat với ba chấm nhún, dòng trạng thái xoay khi chờ lâu (`Em làm ngay đây ạ!` rồi `Em đang tìm thông tin trong não và sớm trả lời...`, `Chờ em một chút, sắp xong rồi ạ...`, `Cảm ơn vì đã chờ em...`; khi có tool thì `✓ Nhận data - đang phân tích...`) và đồng hồ đếm giây (số giây chỉ hiện từ giây thứ 3 trở đi). Nếu TTS đang bật, Javis đọc từng câu chờ để bớt cảm giác im lặng.
