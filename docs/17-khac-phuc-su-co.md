...
| **`agy: command not found`** sau khi cài Antigravity | Session cũ chưa nạp PATH. Chạy `export PATH="$HOME/.local/bin:$PATH"` hoặc mở terminal mới; thử `~/.local/bin/agy`. Nhiều VPS đã có `/usr/local/bin/agy`. Chi tiết: [30 - Nhiều bản & đấu nối bộ não](30-nhieu-ban-va-dau-nao.md). |
| Tenant khác không có skill vừa thêm trên manager | Skill không tự lan. Trên bản manager: **Tổng quan** hoặc **Cài đặt** → **Đồng bộ ngay**. |
...
| Cuộc họp: nói gần mic ra chữ, họp online qua loa thì không | Tick **Ghi tiếng máy**, Bắt đầu. Zoom/Teams app: **Toàn màn hình** + **Chia sẻ âm thanh** (đừng chọn cửa sổ Zoom). Meet trên Chrome: đúng tab họp + chia sẻ âm thanh. **Ctrl+Shift+R** nếu chưa thấy ô tick. |
