...
| `JAVIS_PHAP_CHE_RAG_URL` | Base URL of the legal RAG sidecar (`POST /retrieve`). Empty = local `sources/phap-che` + wiki only | (empty) | Large PDF corpora: sync Drive with `scripts/sync-phap-che-drive.sh`, run LightRAG/RAG-Anything (or stub `scripts/phap_che_rag_sidecar_example.py`), then set the URL. See [Personal legal counsel](28-personal-legal-counsel.md). |
