...
| Meeting: close-mic works, online audio from speakers does not | Meeting mic echo cancellation stays off so analog speakers still reach the mic. Tick **Capture computer audio**, Start, pick the meeting tab or Entire screen + Share audio to mix in digital loopback. **Ctrl+Shift+R** if you do not see the checkbox. |
