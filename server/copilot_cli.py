"""Bộ não GitHub Copilot CLI (`copilot`) - gói Copilot, không phải API key dán tay.
...
def login_huong_dan() -> dict:
    return {
        ...
        "ghi_chu": (
            "Đăng nhập bằng `copilot login`, hoặc đặt `COPILOT_GITHUB_TOKEN`, "
            "`GH_TOKEN`, `GITHUB_TOKEN` cho tiến trình Javis."
        ),
    }
...
def auth_status(bo_qua_cache: bool = False) -> dict:
    """Đã có đường đăng nhập Copilot dùng được chưa."""
    cli = find_copilot_cli()
    if not cli:
        return {
            "connected": False,
    ...
    if ds:
        ...
        d = {
            ...
            "email": "",
            "error": (
                "Đã cài GitHub Copilot CLI nhưng Javis chưa thấy phiên đăng nhập. "
                "Mở terminal trên máy chạy Javis, gõ `copilot login`, hoặc đặt "
    ...
    def _doi_su_kien(self, ev: dict, cac_manh: list[str]) -> list[dict]:
        ...
        if t == "error":
            ...
            if _la_loi_chua_dang_nhap(tin):
                tin = "GitHub Copilot CLI chưa đăng nhập. Gõ `copilot login` hoặc đặt token GitHub vào môi trường."
