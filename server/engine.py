...
OLLAMA_BASE = os.getenv("OLLAMA_BASE", "https://ollama.com")
...
async def _openai_compat_stream(url, label, api_key, model, messages, reasoning, send_reasoning,
    ...
    # Ollama Cloud cần Bearer; Ollama local (máy nhà / http://...) thì không.
    skip_auth = label == "Ollama (Local)" or (label == "Ollama" and not _ollama_needs_auth())
    if not skip_auth:
...
async def single_tool_plan(provider, api_key, model, messages, reasoning, tool_spec):
    ...
    if provider == "openrouter":
        headers.update({"HTTP-Referer": "http://localhost:7777", "X-Title": "VMOS"})
    payload = {
...
async def openrouter_chat_with_mcp(api_key, model, messages, reasoning, mcp_tools, mcp_route):
    headers = {"Authorization": f"Bearer {api_key}", "Content-Type": "application/json",
               "HTTP-Referer": "http://localhost:7777", "X-Title": "VMOS"}
    extra = {}
    if reasoning not in (None, "", "off"):
