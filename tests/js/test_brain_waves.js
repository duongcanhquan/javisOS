...
check("CSS #brainWaves rõ hơn (opacity >= 0.7)", /#brainWaves[\s\S]{0,220}opacity:\s*0\.(7|8|9)/.test(CSS));
check("JS alphaMul Flow rõ hơn (>= 0.6)", /alphaMul = mode === "calm" \? 0\.\d+ : mode === "deep" \? 0\.\d+ : 0\.(6|7|8|9)/.test(WAVES));
check("HTML nạp brain-waves.js?v= >= 4", /brain-waves\.js\?v=([4-9]|\d{2,})/.test(HTML));
