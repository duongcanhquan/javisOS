...
check("ảnh lưu ra có gắn nhãn Javis OS", b"Javis OS" in _raw_luu)
...
check("brand_png vẫn gắn được nhãn khi ảnh đã có caBX", b"Javis OS" in _sau)
...
try:
    ...
    check("bật: nhãn phần mềm Javis OS VẪN còn (không đánh tráo danh nghĩa)",
          b"Javis OS" in _byte_go)
finally:
