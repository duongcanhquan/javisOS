#!/usr/bin/env python3
"""Canh seed Bộ Marketing ghi đúng agents/skills/workflows Gemini."""
...
for slug in ("mkt-nghien-cuu", "mkt-kiem-seo", "mkt-viet-seo", "mkt-facebook", "mkt-kiem-chung"):
    p = agents_dir / f"{slug}.md"
    check(f"agent {slug}", p.is_file())
    text = p.read_text(encoding="utf-8")
    check(f"{slug} gemini", "model_provider: gemini" in text)

for sk in ("marketing-hub", "kiem-tra-seo", "viet-bai-seo", "tong-ket-facebook"):
    ...
    check(f"{sk} description ≤150", m and len(m.group(1).strip()) <= 150)
